        ...
        <div className="max-w-[1400px] mx-auto p-4 sm:p-6 pb-20">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
                ...
                <div className="flex gap-2 flex-wrap">
                        <button
                            onClick={async () => {
                                if (!window.confirm('Iniciar Limpeza Bruta? (Vai apagar os Sem Nome, Teste e forçar a Márcia)')) return;
                                
                                const { deleteDoc, doc, updateDoc, setDoc } = await import('firebase/firestore');
                                const { db } = await import('../firebase');
                                
                                let deleted = 0;
                                let updated = 0;
                                
                                // 1. Apagar fantasmas (Sem Nome e teste)
                                for (const item of items) {
                                    const name = (item.title || '').trim().toLowerCase();
                                    if (name === 'sem nome' || name === 'teste') {
                                        try {
                                            if (item.source === 'quote') await deleteDoc(doc(db, 'saved_quotes', item.id));
                                            if (item.source === 'contract') await deleteDoc(doc(db, 'contracts', item.id));
                                            if (item.source === 'queue') await deleteDoc(doc(db, 'production_queue', item.id));
                                            deleted++;
                                        } catch(e) { console.error(e); }
                                    }
                                }
                                
                                // 2. Resolver Márcia, José e Everton (Forçar união e deletar Orçamentos)
                                const targetNames = ['márcia campos nogueira', 'marcia campos nogueira', 'josé guilherme severini', 'jose guilherme severini', 'everton de araujo'];
                                
                                const quotesToNuke = items.filter(i => i.source === 'quote' && targetNames.includes((i.title || '').trim().toLowerCase()));
                                
                                for (const q of quotesToNuke) {
                                    const addr = q.originalData?.location || q.originalData?.customAddress || '';
                                    ...
                                    const matchingItems = items.filter(i => i.source !== 'quote' && targetNames.includes((i.title || '').trim().toLowerCase()));
                                    for (const m of matchingItems) {
                                        if (addr && addr !== 'N/A') {
                                            try {
                                                if (m.source === 'contract') await updateDoc(doc(db, 'contracts', m.id), { customAddress: addr, location: addr });
                                                if (m.source === 'queue') await updateDoc(doc(db, 'production_queue', m.id), { location: addr });
                                                updated++;
                                            } catch(e) {}
                                        }
                                        
                                        if (m.source === 'contract') {
                                            const hasQ = items.find(x => x.source === 'queue' && (x.originalData?.contractId === m.id || (x.title||'').toLowerCase() === (m.title||'').toLowerCase()));
                                            if (!hasQ) {
                                                try {
                                                    const newOrder: any = {
                                                        contractId: m.id,
                                                        createdAt: new Date().toISOString(),
                                                        clientName: m.title,
                                                        deliveryDate: m.originalData?.deliveryDate || '',
                                                        downPayment: (Number(m.originalData?.totalValue) || 0) / 2,
                                                        balanceDue: (Number(m.originalData?.totalValue) || 0) / 2,
                                                        status: 'in_queue',
                                                        boardStage: 'contrato',
                                                        location: addr || 'N/A',
                                                        installments: [],
                            ...
                            className="bg-red-600 hover:bg-red-700 text-white border border-red-800 rounded-lg px-4 py-2 text-sm font-bold shadow-sm transition-colors"
                        >
                            Faxina Bruta (Márcia e Sem Nome)
                        </button>
                    <select 
                        value={stageFilter}
