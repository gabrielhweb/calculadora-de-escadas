...
export const generateContractPDF = (data: ContractData) => {
  ...
  if (data.selectedOption.landings.length > 0) {
      data.selectedOption.landings.forEach((l, i) => {
          ...
          let dirText = "";
          if (l.direction === 'left') dirText = " (CURVA ESQUERDA)";
          else if (l.direction === 'right') dirText = " (CURVA DIREITA)";
          else dirText = " (RETO)";

          objetoDesc += `-PATAMAR${dirText} EM CHAPA XADREZ 3MM, COM MEDIDAS DE ${lM}M X ${wM}M.\n`;
      });
  }
