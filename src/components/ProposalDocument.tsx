...
const ProposalDocument: React.FC<ProposalDocumentProps> = ({ options, userData, inputData, freightCost, tollCost, installationCost, onBack }) => {
  ...
  const createPdfDoc = useCallback(() => {
    ...
    // --- CABEÇALHO ---
    if (LOGO_BASE64) {
        try { doc.addImage(LOGO_BASE64, 'PNG', (pageWidth / 2) - 15, currentY, 30, 30); } catch (e) {}
    }
    currentY += 40;
    ...
    options.forEach((opt) => {
        ...
        const text1 = `Escada articulada lateral em aço carbono com corte à laser, com medidas de: ${alturaM}m de altura, ${compM}m de comprimento, ${opt.stairWidth}cm de largura e com corrimão de 70cm.`;
        ...
        const text2 = `-Com ${opt.steps} degraus articulados com dimensões de ${opt.stepHeight.toFixed(2).replace('.', ',')}cm de altura e pisante de ${opt.treadDepth.toFixed(2).replace('.', ',')}cm com ${inputData.dampers} amortecedores de alívio.`;
        ...
        // VALOR ESCADA (Sem o patamar para detalhamento se necessário, ou já com ele se for o total da peça)
        // Na imagem o Valor Escada vem primeiro
        const structureOnly = opt.totalPrice - (opt.landing?.active ? opt.landing.price : 0);
        doc.text(`-Valor Escada: ${formatCurrencyBRL(structureOnly)}`, pageMargin, currentY);
        currentY += 6;

        // PATAMAR (EXATAMENTE COMO NA IMAGEM)
        if (opt.landing?.active) {
            const lM = (opt.landing.length / 100).toFixed(2).replace('.', ',');
            const wM = (opt.landing.width / 100).toFixed(2).replace('.', ',');
            doc.setFont('helvetica', 'bold');
            doc.text(`-PATAMAR EM CHAPA XADREZ 3MM, COM MEDIDAS DE ${lM}M X ${wM}M:`, pageMargin, currentY);
            currentY += 6;
            doc.setFont('helvetica', 'normal');
            doc.text(`-Valor Patamar ${formatCurrencyBRL(opt.landing.price)}`, pageMargin, currentY);
            currentY += 6;
        }
        ...
        const instText = installationCost > 0 ? `-Instalação ${formatCurrencyBRL(installationCost)} (Valor para local de fácil acesso)` : `-SEM INSTALAÇÃO (Responsabilidade do Cliente)`;
        doc.text(instText, pageMargin, currentY);
        currentY += 6;
    ...
    currentY += 6;
    doc.text('-Capacidade máxima por degrau: 180k | -Capacidade máxima da escada: 360k', pageMargin, currentY);
    currentY += 10;
    doc.text('Formas de pagamento:', pageMargin, currentY);
    currentY += 6;
    doc.setFont('helvetica', 'normal');
    doc.text('À vista: 5% de desconto (50% sinal + 50% na entrega)', pageMargin, currentY);
    currentY += 5;
    doc.text('À prazo em ate 12x no cartão via link (juros da operadora)', pageMargin, currentY);
    currentY += 10;
    doc.text('Prazo de entrega: 20 dias uteis após pagamento do sinal.', pageMargin, currentY);
    currentY += 6;
